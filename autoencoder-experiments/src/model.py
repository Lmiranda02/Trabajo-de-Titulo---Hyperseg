...
class UNetAutoencoder(nn.Module):
    ...
    def forward(self, x):
        ...
        d2_resized = F.interpolate(d2, size=x1.shape[2:], mode='bilinear', align_corners=False)  # Redimensionar d2 para que tenga las mismas dimensiones que x1
        reconstructed = self.sigmoid(self.dec3(d2_resized + x1))  # (batch, input_channels, H, W)
